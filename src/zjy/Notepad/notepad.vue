<template>
  ...
  <el-calendar ref="calendar" v-model="value">
    ...
    <template #header="{ date }">
      ...
      <el-button-group>
        ...
        <el-button size="mini" @click="selectDate('prev-month')"
          >上一月</el-button
        >
        <el-button size="mini" @click="selectDate('today')">回到今日</el-button>
	...
	<span v-for="NotepadData in NotepadData" >
		...
		<span v-if="NotepadData.beginTime==data.day" style="color:blue;text-decoration:underline" @click="updatenotepad(NotepadData)">{{NotepadData.notepadTheme[0]+"..."}}</span>&nbsp;
	</span>

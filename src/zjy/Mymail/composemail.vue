<template>
  ...
  <el-button type="primary" @click="qrfs('ruleForm')" style="background-color: blue; margin-top: 2%"
    >确认发送</el-button
  >
  <el-button type="primary" @click="bccg('ruleForm')" style="background-color: blue; margin-top: 2%"
    >保存草稿</el-button
  >
  </el-form>
  <el-dialog
  ...
  this.deptId=this.DeptId[this.DeptId.length-1]
    ...
    qrfs(formName) {   //点击确认发送添加数据
     ...
     this.$refs[formName].validate((valid) => {
      ...
      this.axios.post("http://localhost:8088/TSM/send/addsend", {  //添加发件箱
        ...
        .then(function (res) {
          ...
          if (sessionStorage.getItem("qkcs") == "delectdarfts") {     //如果参数为草稿邮件发出后删除草稿箱
            _this.axios.post("http://localhost:8088/TSM/draft/delectdraft", {
                draftId: id,
              })
              .then((res) => {

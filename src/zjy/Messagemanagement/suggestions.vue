<template>
  <div style="margin-top: 2%" class="diva">
    ...
    <div class="b">
      快速检索:<select v-model="pageInfo.js" class="c">
        <option value="意见箱名称">
          <el-icon><delete-filled /></el-icon>意见箱名称
        </option>
        <option value="用户范围">
          <el-icon><delete-filled /></el-icon>用户范围
        </option>
      </select>
    </div>
    <el-input
  ...
  <div class="e">
    <el-button
      ...
      ><el-icon><delete-filled /></el-icon>新增</el-button
    >
    <el-button
      style="background-color: blue; color: white"
      :icon="CircleCloseFilled"
      @click="enablesuggestions(yjxsz)"
      ><el-icon><delete-filled /></el-icon>启用</el-button
    >
    <el-button
      style="background-color: blue; color: white"
      :icon="CircleCloseFilled"
      @click="disablesuggestions(yjxsz)"
      ><el-icon><delete-filled /></el-icon>禁用</el-button
    >
  </div>
  <el-table
    border
  ...
  >
    ...
    <el-table-column prop="suggestionsName" label="意见箱名称" width="300">
      <template #default="scope">
        <span class="a" @click="cknr(scope.row), (xgyjx = true), cxyh()">{{
          scope.row.suggestionsName
        }}</span>
      </template>
    </el-table-column>
  ...
  <el-dialog v-model="xzyjx" title="新增意见箱" width="70%" center>
    ...
    >
      <el-form-item
        label="意见箱名称:"
        style="float: left"
        prop="suggestionsName"
      >
        <el-input
          v-model="ruleForm.suggestionsName"
          style="width: 200px"
        ></el-input>
      </el-form-item>
      ...
      <el-form-item label="用户范围:" prop="userScopesz">
        <el-input
          v-model="ruleForm.userScopesz"
          readonly="readonly"
          @click="(centerDialogVisible = true), qk(), cx(), selectDept()"
        ></el-input>
      </el-form-item>
    ...
    <template #footer>
      ...
      <span class="dialog-footer">
        <el-button type="primary" @click="xzjm = false"> 关闭</el-button>
      </span>
    </template>
  </el-dialog>
  <!-- 修改界面-->
  <el-dialog v-model="xgyjx" title="修改意见箱" width="70%" center>
    ...
    <template #footer>
      <span class="dialog-footer">
        <el-button
          type="primary"
          @click="updatesuggestions('ruleForm'), (xgyjx = false)"
        >
          保存</el-button
        >
      </span>
  ...
  >
    ...
    <el-checkbox-group v-model="yhsz">
      <el-checkbox
        v-for="staffData1 in staffData1"
        :label="staffData1.staffName"
        :key="staffData1.staffName"
        ><span>{{ staffData1.staffName }}</span></el-checkbox
      >
    </el-checkbox-group>
  </el-dialog>
</template>
...
export default {
  ...
  methods: {
    enable(row) {
      ...
      let _this = this;
      this.axios
        .post("http://localhost:8088/TSM/suggestions/updateSuggestions", {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if (this.ruleForm.userScope.length > 0) {
            this.ruleForm.userScope = this.ruleForm.userScope.substr(
              0,
              this.ruleForm.userScope.length - 1
            );
          }
          this.axios
            .post("http://localhost:8088/TSM/suggestions/updateSuggestions", {
            ...
            .then((response) => {
              (_this.ruleForm.suggestionsId = ""),
                (_this.ruleForm.suggestionsName = "");
              _this.ruleForm.staffId = "";
              _this.ruleForm.userScope = [];
              _this.yhsz = [];
              _this.ruleForm.remarks = ""; //清空所有数据
    ...
    addsuggestions(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.axios
            .post("http://localhost:8088/TSM/suggestions/addSuggestions", {
              suggestionsName: this.ruleForm.suggestionsName,
              staffId: this.ruleForm.staffId,
              userScope: this.ruleForm.userScope,
              remarks: this.ruleForm.remarks,
    ...
    creat() {
      var _this = this;
      this.axios
        .get("http://localhost:8088/TSM/suggestions/suggestions", {

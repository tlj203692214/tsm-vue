<template>
  ...
  <div class="nav_top">
    ...
    <el-button @click="insertPopup(), selectPid()">
      <plus style="width: 1em; height: 1em; margin-right: 8px"/>添加权限</el-button>
  </div>
  <!-- 权限表格显示数据 -->
      ...
      <el-table-column label="操作">
        <template #default="scope">
          <el-button type="text" @click="updatePopup(scope.row)"
            >编辑</el-button
          >
          <el-button type="text" @click="deletePopup(scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
<el-dialog v-model="menuDel" title="提示" width="30%">
			<span>确定【{{navInfo.navigationName}}】删除？</span>
			<template #footer>
